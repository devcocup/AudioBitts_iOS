...
- (void) sendCommand:(int)i {
    ...
    [request performRequestWithHandler:^(NSData *responseData, NSHTTPURLResponse *urlResponse, NSError *error) {
        ...
        if ([cmdStr isEqualToString:@"FINALIZE"]) {
            ...
            NSMutableDictionary *processingInfo = returnedData[@"processing_info"];
            NSString *state = processingInfo[@"state"];
            int delaySecs = [processingInfo[@"check_after_secs"] intValue];
            if ([state isEqualToString:@"pending"]) {
                [self performUpdateRequest:delaySecs completionHandler:^{
                    if (error) {
                        NSLog(@"Error: %@", error);
                    } else {
                        ...
                        if (!is2XX) {
                            ...
                            NSError *serror = nil;
                            NSMutableDictionary *returnedData = [NSJSONSerialization JSONObjectWithData:responseData options:NSJSONReadingMutableContainers error:&serror];
...
//- (void) performUpdateRequest:(int) delaySecs completionHandler:(void(^)())handler {
//    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delaySecs * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
...
//            int delaySecs = [processingInfo[@"check_after_secs"] intValue];
//            if ([state isEqualToString:@"pending"]) {
//                [self performUpdateRequest:delaySecs completionHandler:handler];
//            } else {
//                handler();
//            }
//        }];
...
- (void) shareToTwitter:(NSData *)shareData params:(NSDictionary *)params
{
//    NSString *text = params[@"text"];
//    NSString *lengthVideo = [NSString stringWithFormat:@"%lu", (unsigned long)shareData.length];
...
//        NSString *secret = [Twitter sharedInstance].sessionStore.session.authTokenSecret;
...
//        Twitter *twitterInstance = [Twitter sharedInstance];
//        
//        NSError *error;
